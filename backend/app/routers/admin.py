...
def crear_reserva(payload: ReservaCreate):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        sala = _obtener_sala(cursor, payload.id_sala)
        if not sala:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "La sala no existe")

        participantes = _validar_reserva(cursor, sala, payload.fecha, payload.id_turno, payload.participantes)

        cursor.execute(
            "INSERT INTO reserva(id_sala, fecha, id_turno, estado) VALUES (%s, %s, %s, 'activa')",
            (payload.id_sala, payload.fecha, payload.id_turno),
        )
        reserva_id = cursor.lastrowid
        _guardar_participantes(cursor, reserva_id, participantes)
...
def registrar_asistencia(id_reserva: int, payload: AsistenciaPayload):
    if not payload.registros:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Debe indicar la asistencia")
    ...
    try:
        ...
        participantes = _participantes_reserva(cursor, id_reserva)
        if not participantes:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "La reserva no tiene participantes")
        ...
        for registro in payload.registros:
            cursor.execute(
                ...
                """,
                (registro.asistencia, id_reserva, registro.ci_participante),
            )
            if cursor.rowcount == 0:
                raise HTTPException(status.HTTP_400_BAD_REQUEST, "El participante no pertenece a la reserva")
            if registro.asistencia:
                presentes += 1

        if presentes == 0:
            cursor.execute("UPDATE reserva SET estado = 'sin_asistencia' WHERE id_reserva = %s", (id_reserva,))
            fin = _sumar_meses(reserva["fecha"], 2)
            for ci in participantes:
                if not _sancion_solapada(cursor, ci, reserva["fecha"], fin):
                    _insertar_sancion(cursor, ci, reserva["fecha"], fin)
...
# Sanciones
@router.post("/sanciones", status_code=status.HTTP_201_CREATED)
def crear_sancion(payload: SancionCreate):
    _validar_ci(payload.ci_participante)
    if payload.fecha_fin <= payload.fecha_inicio:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "La fecha fin debe ser mayor")
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor(dictionary=True)
        if not _obtener_participante(cursor, payload.ci_participante):
            raise HTTPException(status.HTTP_404_NOT_FOUND, "El participante no existe")
        if _sancion_solapada(cursor, payload.ci_participante, payload.fecha_inicio, payload.fecha_fin):
            raise HTTPException(status.HTTP_409_CONFLICT, "La sanción se superpone con otra")

        _insertar_sancion(cursor, payload.ci_participante, payload.fecha_inicio, payload.fecha_fin)
        conn.commit()
...
def actualizar_sancion(payload: SancionUpdate):
    _validar_ci(payload.ci_participante)
    if payload.nueva_fecha_fin <= payload.fecha_inicio:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "La nueva fecha debe ser mayor")
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        cursor.execute(
            "SELECT fecha_inicio, fecha_fin FROM sancion_participante WHERE ci_participante = %s AND fecha_inicio = %s",
            (payload.ci_participante, payload.fecha_inicio),
        )
        ...
        if _sancion_solapada(cursor, payload.ci_participante, payload.fecha_inicio, payload.nueva_fecha_fin):
            raise HTTPException(status.HTTP_409_CONFLICT, "La nueva vigencia se superpone con otra sanción")

        _insertar_sancion(cursor, payload.ci_participante, payload.fecha_inicio, payload.nueva_fecha_fin)
        conn.commit()
...
def eliminar_sancion(ci: str, fecha_inicio: date):
    _validar_ci(ci)
    conn = None
    cursor = None
    try:
